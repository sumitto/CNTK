...
namespace CNTK
{
    ...
    /*static*/ ValuePtr Value::CreateBatch(const NDShape& sampleShape, const std::vector<ElementType>& batchData, const DeviceDescriptor& device, bool readOnly /*= false */)
    {
        ...
        std::vector<NDArrayViewPtr> sequencesData(numOfSequences);
        for (size_t i = 0; i < numOfSequences; i++)
        {
            // Sequence lenght is 1.
            auto sequenceDataShape = sampleShape.AppendShape({ 1 });
            sequencesData[i] = MakeSharedObject<NDArrayView>(sequenceDataShape, batchData.data() + i * shapeSize, shapeSize, DeviceDescriptor::CPUDevice());
        }
        // Pass the empty seqStartFlags means all sequences have the start flag with true.
        return Create(sampleShape, sequencesData, {}, device, readOnly, /*createNewCopy =*/ true);
    }
    ...
    {
        std::vector<std::vector<ElementType>> input = {sequenceData};
